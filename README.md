...
LLMs training in simple, pure C/CUDA with no need for 245MB of PyTorch or 107MB of cPython. Current focus is on pretraining multiple architectures including:
- [GPT-2](https://github.com/openai/gpt-2) and [GPT-3](https://arxiv.org/abs/2005.14165) series
- **Qwen3**: Alibaba's Qwen3 model series with improved multilingual capabilities
- **Qwen3_NEXT**: Next generation Qwen architecture with enhanced performance

This repo provides pure CUDA implementations with parallel PyTorch reference implementations. The bleeding edge mainline code includes optimized CUDA kernels and a simple reference CPU fp32 implementation in ~1,000 lines of clean code. We maintain C and CUDA code for maximum performance and educational value.

## quick start

The best introduction to this repo is reproducing the GPT-2 (124M) model. We support training multiple architectures:
- **GPT-2 and GPT-3 series**: Original transformer-based models
- **Qwen3**: Train Qwen3 models with CUDA acceleration (see [train_qwen3_fp32.cu](train_qwen3_fp32.cu))
- **Qwen3_NEXT**: Next generation Qwen architecture with improved efficiency

All models can be trained in both pure CUDA and parallel PyTorch implementations. Have a look at the [scripts README](scripts/README.md) for detailed training scripts.
...
If you won't be training on multiple nodes, aren't interested in mixed precision, and are interested in learning CUDA, the fp32 files are perfect for you. These are simpler, more portable, and easier to understand.
...
**For Qwen3 training:**
```bash
# Train Qwen3 with fp32 precision and context parallelism
nvcc -O3 -o train_qwen3_fp32 train_qwen3_fp32.cu -lcublas
./train_qwen3_fp32
```

**For Qwen3_NEXT training:**
```bash
# Train Qwen3_NEXT with optimized kernels
# (Coming soon - enhanced architecture with improved performance)
```
